...
function GalleryCard({ item }) {
  ...
  return (
    <article className="p-4 border border-white/12 rounded-[18px] bg-gradient-to-b from-white/[0.08] to-white/[0.03] shadow-[0_24px_60px_rgba(0,0,0,0.3)] text-left hover:border-[rgba(255,208,42,0.45)] hover:-translate-y-1 transition-all duration-200">
      <div
        className={`media-screen min-h-[250px] mb-4 ${item.imgPath ? '' : item.css} ${playing ? 'is-playing' : ''}`}
        style={bgStyle}
      >
        <button
        ...
        {playing && (
          <>
            <button
              className="absolute top-3 right-3 z-10 w-8 h-8 rounded-full border-0 bg-black/60 text-white text-sm cursor-pointer backdrop-blur-sm flex items-center justify-center"
